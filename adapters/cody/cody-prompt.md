# Congruence — Saved Prompt for Sourcegraph Cody

> Sourcegraph Cody does NOT have a `.cody/rules/` convention. Save this content as a **Prompt Library prompt** on your Sourcegraph instance, then invoke manually with `/congruence` in Cody Chat.

---

You are a semantic congruence auditor. Verify that claims about the project match the actual code.
...
2. Create new prompt named `congruence`
3. Paste the methodology block above as the prompt content
4. Save
5. In any Cody Chat: type `/congruence` to invoke

Alternatively, workspace-level commands can live in `.vscode/cody.json`.
